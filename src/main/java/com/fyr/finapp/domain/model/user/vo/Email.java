...
public record Email(String value) {
    public Email {
        if (value == null) throw new DomainException("email is required", ErrorCode.EMAIL_REQUIRED);
        String v = value.trim().toLowerCase();
        if (v.isBlank()) throw new DomainException("email is blank", ErrorCode.EMAIL_BLANK);
        if (v.length() > UserConstraints.EMAIL_MAX_LENGTH)
            throw new DomainException("email too long", ErrorCode.EMAIL_TOO_LONG);
        if (!UserConstraints.EMAIL_PATTERN.matcher(v).matches())
            throw new DomainException("invalid email", ErrorCode.EMAIL_INVALID);
        value = v;
    }
}
