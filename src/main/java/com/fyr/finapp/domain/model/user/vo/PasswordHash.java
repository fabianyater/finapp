...
public record PasswordHash(String value) {
    public PasswordHash {
        if (value == null)
            throw new DomainException("passwordHash is required", ErrorCode.PASSWORD_HASH_REQUIRED);
        String v = value.trim();
        if (v.isBlank())
            throw new DomainException("passwordHash is blank", ErrorCode.PASSWORD_HASH_BLANK);
        if (v.length() < UserConstraints.HASHED_PASSWORD_MIN_LENGTH)
            throw new DomainException("passwordHash too short", ErrorCode.PASSWORD_HASH_TOO_SHORT);
        value = v;
    }
}
